...
<script type="text/javascript">
	...
	function selectOrg() {
		window.open("person_selectOrg.action?orgId=" + ${orgId}, "选择组织", "height=650,width=1200,top=0,left=83,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
		...
		<div class="content">
			<div class="org-wrap">
				...
				<c:if test="${!empty person }">
					...
					<form action="person_update.action?person.id=${person.id }" method="post" class="add-input">
						<input type="text" name="person.name" placeholder="${person.name }" class="input-add-org" /></br> 
						
						<c:if test="${person.sex eq '男' }">
							男&nbsp;<input type="radio" name="person.sex" value="男" checked="checked" />&nbsp;
							女&nbsp;<input type="radio" name="person.sex" value="女"/></br>
						</c:if>
						
						<c:if test="${person.sex eq '女' }">
							男&nbsp;<input type="radio" name="person.sex" value="男"/>&nbsp;
							女&nbsp;<input type="radio" name="person.sex" value="女" checked="checked" /></br>
						</c:if>
						
						<input type="text" name="person.duty" placeholder="${person.duty }" class="input-add-org" /></br> 
						<input type="text" name="birthday" onclick="fPopCalendar(event,this,this)" placeholder="${person.birthday }" class="input-add-org" /></br> 
						<input type="text" name="person.address" placeholder="${person.address }" class="input-add-org" /></br> 
						<input type="text" name="person.phone" placeholder="${person.phone }" class="input-add-org" /></br> 
						
						<input type="hidden" name="orgId" id="orgId">
						<input type="text" name="orgName" placeholder="${person.organization.name }" id="orgName"
							onclick="selectOrg();" class="input-add-org" /></br> 
